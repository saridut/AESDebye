...
public:
  ...
  Profile() = default;
  Profile(std::vector<double> q, std::vector<double> intensity,
          std::vector<double> twoTheta, double calculationTime, bool testPassed)
      : q(std::move(q)), intensity(std::move(intensity)),
        twoTheta(std::move(twoTheta)), calculationTime(calculationTime),
        testPassed(testPassed) {}

  Profile(double start, double end, size_t nSteps, bool twoThetaSpace,
          double lambda) {
...
#pragma omp parallel for schedule(static)                                      \
    shared(q, twoTheta, twoThetaSpace, start, end, nSteps, stepSize, lambda,   \
               warn) default(none)
    ...
    if (warn) {
      std::cerr << "Warning: During the conversion from q to theta, some "
                   "values are outside the valid range for the given lambda. "
                << "Some values have been clipped to the valid range."
                << std::endl;
    }
  }
